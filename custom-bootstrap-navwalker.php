...
/**
 * Class Name: Custom_Bootstrap_Navwalker
 ...
 * using the WordPress built in menu manager. This class has also been altered to deviate from ASU Web Standards by
 * allowing top-level menu items to be clickable. (ASU Web Standards specify that the top-level item in a dropdown menu
 * must not click through to another page. For usability and accessibility reasons, the top-level menu item is only used
 * to reveal the dropdown choices.)
 *
 * Version: 2.0.4
  ...
  public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
    ...
    if ( 0 == strcasecmp( $item->attr_title, 'divider' ) &&  1 === $depth ) {
      ...
      if ( $args->has_children && 0 === $depth  ) {
        // EDIT: href is not overwritten with '#' so top-level menu items are not disabled
